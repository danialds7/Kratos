    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    // Get unknown variable from convection diffusion sediment settings
    const ProcessInfo& r_process_info = rCurrentProcessInfo; // To ensure that Gets are threadsafe
    ConvectionDiffusionSedimentSettings &r_conv_diff_sed_settings = *(r_process_info[CONVECTION_DIFFUSION_SEDIMENT_SETTINGS]);
    const Variable<double> &r_unknown_var = r_conv_diff_sed_settings.GetUnknownVariable();
    ...
    ConditionDataStruct &rData)
{
    ...
    ConvectionDiffusionSedimentSettings &r_conv_diff_sed_settings = *(rCurrentProcessInfo[CONVECTION_DIFFUSION_SEDIMENT_SETTINGS]);
    const Variable<double> &r_unknown_var = r_conv_diff_sed_settings.GetUnknownVariable();
    const Variable<double> &r_flux_var = r_conv_diff_sed_settings.GetSurfaceSourceVariable();
    const auto &r_geometry = this->GetGeometry();
    const unsigned int n_nodes = r_geometry.PointsNumber();
