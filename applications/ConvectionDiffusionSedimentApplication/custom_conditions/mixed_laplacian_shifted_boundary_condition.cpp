    ...
    const ProcessInfo& rCurrentProcessInfo) const
{
    ...
    auto& r_conv_diff_sed_settings = *(rCurrentProcessInfo[CONVECTION_DIFFUSION_SEDIMENT_SETTINGS]);
    const auto& r_unknown_var = r_conv_diff_sed_settings.GetUnknownVariable();
    const auto& r_gradient_var = r_conv_diff_sed_settings.GetGradientVariable();
    ...
    const ProcessInfo& rCurrentProcessInfo)
{
    ...
    const auto& r_unknown_var = r_conv_diff_sed_settings.GetUnknownVariable();
    const auto& r_gradient_var = r_conv_diff_sed_settings.GetGradientVariable();
    const auto& r_flux_var = r_conv_diff_sed_settings.GetSurfaceSourceVariable();
    const auto& r_diffusivity_var = r_conv_diff_sed_settings.GetDiffusionVariable();
