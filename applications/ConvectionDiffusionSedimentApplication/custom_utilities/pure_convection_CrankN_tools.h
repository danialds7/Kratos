...
public:
                          ...
                          unsigned int time_order )
    {
        ...
        array_1d<double,TDim> vel_gauss_old;
...
//		unsigned int dof_position = (model_part.NodesBegin())->GetDofPosition(rScalarVar);
                ...
                i!=model_part.ElementsEnd(); i++)
        {
            ...
            noalias(vel_gauss_old) = ZeroVector(TDim);
            double proj = 0.0;
            double proj_old = 0.0;
            for(unsigned int i = 0; i<number_of_points; i++)
            {
                ...
                const array_1d<double,3>& v_old = geom[i].FastGetSolutionStepValue(rTransportVel,1);
                const array_1d<double,3>& w_old = geom[i].FastGetSolutionStepValue(rMeshVel,1);
                proj += geom[i].FastGetSolutionStepValue(rProjVar);
                proj_old += geom[i].FastGetSolutionStepValue(rProjVar,1);
                for(unsigned int j = 0; j<TDim; j++)
                {
                    vel_gauss[j] += v[j] - w[j];
                    vel_gauss_old[j] += v_old[j] - w_old[j];
                }
            }
            //change of meaning of vel_gauss; vel_gauss in n+1/2;
            vel_gauss += vel_gauss_old;
            vel_gauss *= lumping_factor*0.5;

            //change of meaning of proj: proj= proj_n+1/2;
            proj += proj_old;
            proj *= lumping_factor*0.5;
            ...
            noalias(lhs_contribution) = 0.5*outer_prod(N,u_DN);


            //CONVECTION STABILIZING CONTRIBUTION (Suu)
            double tau = CalculateTAU(vel_gauss,Volume);
            noalias(lhs_contribution) += 0.5*(tau) * outer_prod(u_DN,u_DN);
            ...
            // RHS += M*vhistory
            //calculating the historical velocity (rScalarVar,1 = vel previous step)
            for(unsigned int iii = 0; iii<number_of_points; iii++)
            {
            ...
            // RHS -= LHS*rScalarVars  ****RHS -= LHS*(v_n+1-v_n)  (0.5 already included in lhs)
            for(unsigned int iii = 0; iii<number_of_points; iii++)
            {
                temp_vec_np[iii] = geom[iii].FastGetSolutionStepValue(rScalarVar)+geom[iii].FastGetSolutionStepValue(rScalarVar,1);
            }
            ...
            for(unsigned int iii = 0; iii<number_of_points; iii++)
            {
                rhs_contribution[iii] += MassFactors[iii]*BDFcoeffs[0]*geom[iii].FastGetSolutionStepValue(rScalarVar,1);
            }
