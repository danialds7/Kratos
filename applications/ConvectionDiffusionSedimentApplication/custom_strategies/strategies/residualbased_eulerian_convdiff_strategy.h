...
#if !defined(KRATOS_RESIDUALBASED_EULERIAN_CONVECTION_DIFFUSION_SEDIMENT_STRATEGY )
#define  KRATOS_RESIDUALBASED_EULERIAN_CONVECTION_DIFFUSION_SEDIMENT_STRATEGY
...
The convection-diffusion-sediment problem
...
template<class TSparseSpace,
         ...
         class TLinearSolver
         >
class ResidualBasedEulerianConvectionDiffusionSedimentStrategy
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION( ResidualBasedEulerianConvectionDiffusionSedimentStrategy );
    ...
    */
    ...
    ResidualBasedEulerianConvectionDiffusionSedimentStrategy(
...
//			mstep1->SetEchoLevel(2);
    ...
    /** Destructor.
    */
    virtual ~ResidualBasedEulerianConvectionDiffusionSedimentStrategy()
    {
    ...
    */
    ...
    int Check() override
    {
        ...
        if (rCurrentProcessInfo.Has(CONVECTION_DIFFUSION_SEDIMENT_SETTINGS)==false)
            KRATOS_ERROR << "no CONVECTION_DIFFUSION_SEDIMENT_SETTINGS in model_part" << std::endl;
  ...
  virtual void GenerateMeshPart(int dimension)
  {
        ...
        for(ModelPart::ElementsContainerType::iterator it= BaseType::GetModelPart().ElementsBegin(); it != BaseType::GetModelPart().ElementsEnd(); ++it)
         {
            ...
            if (NumNodes == 3)
            {
              pElem = Element::Pointer(new EulerianConvectionDiffusionSedimentElement<2,3>(
                      (*it).Id(),
                      (*it).pGetGeometry(),
                      (*it).pGetProperties() ) );
            ...
            {
                pElem = Element::Pointer(new EulerianConvectionDiffusionSedimentElement<2,4>(
                      (*it).Id(),
                      (*it).pGetGeometry(),
                      (*it).pGetProperties() ) );
              MeshElems.push_back(pElem);
    ...
    else
    {
        for(ModelPart::ElementsContainerType::iterator it= BaseType::GetModelPart().ElementsBegin(); it != BaseType::GetModelPart().ElementsEnd(); ++it)
        {
            ...
            if (NumNodes == 4)
            {
              pElem = Element::Pointer(new EulerianConvectionDiffusionSedimentElement<3,4>(
                      (*it).Id(),
                      (*it).pGetGeometry(),
                      (*it).pGetProperties() ) );
              MeshElems.push_back(pElem);
            ...
            {
                pElem = Element::Pointer(new EulerianConvectionDiffusionSedimentElement<3,8>(
                      (*it).Id(),
                      (*it).pGetGeometry(),
                      (*it).pGetProperties() ) );
              MeshElems.push_back(pElem);
...
private:
    ...
    /** Copy constructor.
    */
    ResidualBasedEulerianConvectionDiffusionSedimentStrategy(const ResidualBasedEulerianConvectionDiffusionSedimentStrategy& Other);
...
#endif /* KRATOS_RESIDUALBASED_EULERIAN_CONVECTION_DIFFUSION_SEDIMENT_STRATEGY  defined */
