...
#if !defined(KRATOS_RESIDUALBASED_SEMI_EULERIAN_CONVECTION_DIFFUSION_SEDIMENT_STRATEGY )
#define  KRATOS_RESIDUALBASED_SEMI_EULERIAN_CONVECTION_DIFFUSION_SEDIMENT_STRATEGY
...
//#include "custom_utilities/convection_diffusion_sediment_settings.h"
...
/// This strategy is used to solve convection-diffusion-sediment problem.
/**   Detail class definition.

Exactly the same as the ResidualBasedEulerianConvectionDiffusionSedimentStrategy, except for the fact that the assembled elements are only transient diffusion elements.
...
template<class TSparseSpace,
         ...
         class TLinearSolver
         >
class ResidualBasedSemiEulerianConvectionDiffusionSedimentStrategy
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION( ResidualBasedSemiEulerianConvectionDiffusionSedimentStrategy );
    ...
    */
    ...
    ResidualBasedSemiEulerianConvectionDiffusionSedimentStrategy(
        ...
        mrReferenceModelPart(model_part)
    {
        ...
        if(mrReferenceModelPart.GetModel().HasModelPart("ConvectionDiffusionSedimentPart"))
            KRATOS_ERROR << "ConvectionDiffusionSedimentPart already exists when constructing ResidualBasedSemiEulerianConvectionDiffusionSedimentStrategy" << std::endl;
        ...
        //ConvectionDiffusionSedimentSettings::Pointer my_settings = rCurrentProcessInfo.GetValue(CONVECTION_DIFFUSION_SEDIMENT_SETTINGS);
        //(mpConvectionModelPart->GetProcessInfo()).GetValue(CONVECTION_DIFFUSION_SEDIMENT_SETTINGS) = my_settings;
		Check();
...
//			mstep1->SetEchoLevel(2);
    ...
    /** Destructor.
    */
    virtual ~ResidualBasedSemiEulerianConvectionDiffusionSedimentStrategy()
    {
        mrReferenceModelPart.GetModel().DeleteModelPart("ConvectionDiffusionSedimentPart");
    }
    ...
    */
    ...
    int Check() override
    {
        KRATOS_TRY
        ProcessInfo& rCurrentProcessInfo = BaseType::GetModelPart().GetProcessInfo();
        if (rCurrentProcessInfo.Has(CONVECTION_DIFFUSION_SEDIMENT_SETTINGS)==false)
			KRATOS_THROW_ERROR(std::logic_error, "no CONVECTION_DIFFUSION_SEDIMENT_SETTINGS in model_part", "");
        //std::cout << "ConvDiff::Check(). If crashes, check CONVECTION_DIFFUSION_SEDIMENT_SETTINGS is defined" << std::endl;
...
private:
  ...
  void GenerateMeshPart(int dimension)
  {
    ...
    mpConvectionModelPart = &(mrReferenceModelPart.GetModel().CreateModelPart("ConvectionDiffusionSedimentPart"));
    ...
    /** Copy constructor.
    */
    ResidualBasedSemiEulerianConvectionDiffusionSedimentStrategy(const ResidualBasedSemiEulerianConvectionDiffusionSedimentStrategy& Other);
...
#endif /* KRATOS_RESIDUALBASED_SEMI_EULERIAN_CONVECTION_DIFFUSION_SEDIMENT_STRATEGY  defined */
