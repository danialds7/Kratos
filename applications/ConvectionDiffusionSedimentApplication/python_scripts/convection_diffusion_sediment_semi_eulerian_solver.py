...
def CreateSolver(model, custom_settings):
    return ConvectionDiffusionSedimentSemiImplicitSolver(model, custom_settings)

class ConvectionDiffusionSedimentSemiImplicitSolver(convection_diffusion_sediment_solver.ConvectionDiffusionSedimentSolver):
    """The semi-eulerian class for convection-diffusion-sediment solvers.
    ...
    def SolveSolutionStep(self):
        ...
        thermal_settings  = self.GetComputingModelPart().ProcessInfo.GetValue(KratosMultiphysics.CONVECTION_DIFFUSION_SEDIMENT_SETTINGS)
    ...
    def _CreateBFECCConvection(self):
        ...
        if domain_size ==2:
            bfecc_utility = ConvectionDiffusionSedimentApplication.BFECCConvection2D(point_locator)
        else:
            bfecc_utility = ConvectionDiffusionSedimentApplication.BFECCConvection3D(point_locator)
