...
import KratosMultiphysics.ConvectionDiffusionSedimentApplication as CDA
...
class MoveParticleUtilityProcess(KM.Process):
    ...
    def Check(self):
        settings = self.model_part.ProcessInfo[KM.CONVECTION_DIFFUSION_SEDIMENT_SETTINGS]
        if not settings.IsDefinedUnknownVariable():
            raise Exception('Unknown variable not defined in convection diffusion sediment settings')
        if not settings.IsDefinedProjectionVariable():
            raise Exception('Projection variable not defined in convection diffusion sediment settings')
        if not settings.IsDefinedVelocityVariable():
            raise Exception('Velocity variable not defined in convection diffusion sediment settings')
        if not settings.IsDefinedMeshVelocityVariable():
            raise Exception('MeshVelocity variable not defined in convection diffusion sediment settings')
