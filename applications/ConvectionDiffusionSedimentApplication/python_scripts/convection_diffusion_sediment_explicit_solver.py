...
def CreateSolver(model, custom_settings):
    return ConvectionDiffusionSedimentExplicitSolver(model, custom_settings)


class ConvectionDiffusionSedimentExplicitSolver(convection_diffusion_sediment_solver.ConvectionDiffusionSedimentSolver):
    """
    The explicit class for convection-diffusion-sediment solvers.
    ...
    def __init__(self, model, custom_settings):
        ...
        if self.settings["use_orthogonal_subscales"].GetBool() is True:
            oss_element_list = ["QSConvectionDiffusionSedimentExplicit", "DConvectionDiffusionSedimentExplicit"]
            if element_name in oss_element_list:
                ...
                err_msg = (
                    ...
                    element_name,
                    "does not support OSS projection. Select QSConvectionDiffusionSedimentExplicit or DConvectionDiffusionSedimentExplicit instead.",
                )
    ...
    def _CreateSolutionStrategy(self):
        convection_diffusion_sediment_solution_strategy = self._create_runge_kutta_4_strategy()
    ...
    def _create_runge_kutta_4_strategy(self):
        ...
        rebuild_level = 0
        return ConvectionDiffusionSedimentApplication.ExplicitSolvingStrategyRungeKutta4ConvectionDiffusionSediment(
