...
//         const double cr_nk = 0.5;
	...
	//if it is a convection diffusion sediment problem, then the projection variable will exist and therefore we must use it instead of unknownVar(timestep n)
...
//             dphi_dt[i] = dt_inv*(phi[i] - phi_old [i];
        ...
        const ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        ConvectionDiffusionSedimentSettings::Pointer p_my_settings = rCurrentProcessInfo.GetValue(CONVECTION_DIFFUSION_SEDIMENT_SETTINGS);
        ...
        for (unsigned int i = 0; i < TNumNodes; i++) {
            ...
            // If it is a convection diffusion sediment problem, then the projection variable will exist and
