                                                    ...
                                                    array_1d<double, 3>& r_contact_force)
{
    ...
    for (int i = 0; data_buffer.SetNextNeighbourOrExit(i); ++i){
        ...
        if (CalculateRelativePositionsOrSkipContact(data_buffer)) {
            ...
            //  std::cout<<"Ball_contact particle Id "<<this->Id()<<" neighbour id "<<data_buffer.mpOtherParticle->Id()<<" "<<LocalElasticContactForce[0]<<" "<<LocalElasticContactForce[1]<<" "<<LocalElasticContactForce[2]<<std::endl<<std::flush;
